	...
	(i = 0),
		((r = function (e, t) {
			...
			d(t.$elements.calendarEl).hasClass("calendar-initialized") ||
				...
				t.windowW <= t.$breakpoints.tablet ? (t.toggleSidebar(!1), t.toggleEventList(!1)) : (t.options.sidebarDisplayDefault ? t.toggleSidebar(!0) : t.toggleSidebar(!1), t.options.eventDisplayDefault ? t.toggleEventList(!0) : t.toggleEventList(!1)),
		...
		(r.prototype.destroy = function () {
			...
			e.destroyEventListener(),
				e.$elements.calendarEl && (e.$elements.calendarEl.removeClass("calendar-initialized"), e.$elements.calendarEl.removeClass("evo-calendar"), e.$elements.calendarEl.removeClass("sidebar-hide"), e.$elements.calendarEl.removeClass("event-hide")),
		...
		(r.prototype.resize = function () {
			...
			(e.windowW = d(window).width()),
				e.windowW <= e.$breakpoints.tablet
					? (e.toggleSidebar(!1), e.toggleEventList(!1), e.windowW <= e.$breakpoints.mobile ? d(window).off("click.evocalendar.evo-" + e.instanceUid) : d(window).on("click.evocalendar.evo-" + e.instanceUid, d.proxy(e.toggleOutside, e)))
			...
			if ((e.calculateDays(), !e.$elements.calendarEl.html())) {
				...
				(t += "</tr></table></div>"),
					...
					e.options.sidebarToggler &&
						(d(e.$elements.sidebarEl).append('<span id="sidebarToggler" role="button" aria-pressed title="' + e.initials.dates[e.options.language].closeSidebarText + '"><button class="icon-button"><span class="bars"></span></button></span>'),
					...
					e.options.eventListToggler &&
						(d(e.$elements.calendarEl).append(
							'<span id="eventListToggler" role="button" aria-pressed title="' + e.initials.dates[e.options.language].closeEventListText + '"><button class="icon-button"><span class="chevron-arrow-right"></span></button></span>'
						),
		...
		(r.prototype.removeEventList = function (e) {
			...
			0 !== n.find('[data-event-index="' + e + '"]').length &&
				...
				0 === n.find("[data-event-index]").length &&
					(n.empty(), a.$active.date === a.$current.date ? (t += "<p>" + a.initials.dates[a.options.language].noEventForToday + "</p>") : (t += "<p>" + a.initials.dates[a.options.language].noEventForThisDay + "</p>"), n.append(t)));
		}),
